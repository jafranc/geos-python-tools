...
class Solver_Sparse(Solver):
    ...
    def __init__(self, version, units):
        ...
        if self.version == 'b':
            ...
            self.schedule.extend(list(np.arange(100 * Conversion.SEC2YEAR, 500 * Conversion.SEC2YEAR, 50 * Conversion.SEC2YEAR)))
            self.schedule.extend(list(
    ...
    def _plot_(self, directory):
        ...
        #
        axs[1][1].plot( df['#t[s]'].to_numpy() / time_unit, df[' tstep[s]'].to_numpy(dtype=float)/time_unit, label=f'tstep[{time_name}]')
        axs[1][1].legend()
